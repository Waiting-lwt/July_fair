			...
			<view class='nvFixed' :style="[{'width':windowWidth+'px'}]">
				...
				<view :class="['nvContent','nvLogoBox',{'androidwx':androidwx}]" v-if="config.type=='logo'">
					...
					<view v-if="config.btn&&config.btn.length>0" class="nvBtnGroup">
						...
						<text class="nvBtn" v-if="b.text" @tap.stop="nvBtnTap" :data-index="n" :data-type="b.type" :style="b.style||{color:getTxtColor}">{{b.text}}</text>
...
<style lang="scss" scoped>
	...
	.nvInput{font-size: 30rpx;flex: 1;}
	.searchIcon{
		width: 18px;
		margin-right: 20rpx;
		/* #ifdef MP */
	...
	.nvBtn,.nvBtnImg{padding: 0;margin-left: 20rpx;background: transparent;border: 0;color: #000000;z-index: 3;font-size: 28rpx;}
	.nvBtnImg{width: 48rpx;padding-left: 0;}
